"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
var DomainException = /** @class */ (function (_super) {
    __extends(DomainException, _super);
    function DomainException(message, parameters) {
        if (parameters === void 0) { parameters = []; }
        var _this = _super.call(this) || this;
        _this._parameters = [];
        if (parameters) {
            _this.message = "message: " + message + ", parameteres: " + parameters.join(', ');
        }
        else {
            _this.message = "message: " + message;
        }
        return _this;
    }
    Object.defineProperty(DomainException.prototype, "parameters", {
        get: function () {
            return this._parameters;
        },
        enumerable: false,
        configurable: true
    });
    return DomainException;
}(Error));
exports.default = DomainException;
//# sourceMappingURL=base.exception.js.map